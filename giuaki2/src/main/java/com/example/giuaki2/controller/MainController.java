package com.example.giuaki2.controller;
...
import com.example.giuaki2.service.BookService;
...
public class MainController {
    ...
    public ResponseEntity<String> createBook(@RequestBody Book newBook) {
        try {
            ...
            return new ResponseEntity<>("Khoi Tao Du Lieu Thanh Cong ."
                    ,HttpStatus.CREATED);
        } catch (Exception e) {
    ...
    public ResponseEntity<String> deleteBook(@RequestParam(name = "isbn") String isbn) {
        try {
            ...
            return new ResponseEntity<>("Da xoa du lieu thanh cong.", HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(e.getMessage(), HttpStatus.BAD_REQUEST);
        }
    }
